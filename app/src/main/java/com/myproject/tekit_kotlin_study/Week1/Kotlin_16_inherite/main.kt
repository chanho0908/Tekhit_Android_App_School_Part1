package com.myproject.tekit_kotlin_study.Week1.Kotlin_16_inherite

// 상속
// 하나의 클래스가 다른 클래스에 작성한 요소를 물려받는 개념
// 클래스를 통해 객체를 생성하면 클래스에 작성한 부분 뿐마 아니라 물려주는 쪽이 가지고 있는 부분도
// 가지고 있게된다.
// 물려 주는 쪽을 부모클래스(Super Class) 라고 부르고 물려 받는 쪽을 자식 클래스 (Sub Class) 라고 부른다.

// 코틀린에서 부모 클래스의 역할을 할 수 있도록 하겠다면 open 키워드를 붙혀줘야 한다.
fun main(){
    // 자식 클래스를 통해 객체를 생성한다.
    val s1 = SubClass1()
    println(s1.superA1)
...
class SubClass2 : SuperClass2{
    constructor(){
        // 부모 클래스의 생성자를 명식으로 호출 하지 않으면
        // 부모 클래스가 가지고 있는 생성자 중 매개변수가 없는 생성자를 자동으로 호출한다.
    }

    constructor(a1: Int): super(100) {
        // 만약 부모가 가지고 있는 생성자 중 매개변수가 있는 다른 생성자를 호출 하겠다면
        // 명시적으로 호출해주면 된다.
...
class SubClass4: SuperClass2(100)