package com.myproject.tekit_kotlin_study.kotlin.Kotlin19_Property
// property : 캡슐화를 적용한 변수(private 접근 제한자를 설정한 변수)에 값을 저장할 수 있는 메서드(setter)
// 나 변수의 값을 반환 하는 메서드(getter)를 제공하는 것
...
class TestClass1{
    ...
    // private가 아닌 접근 제한자를 붙힌 변수는 Java 로 변환될 때 변수의 접근 제한자는 private로 변경된다
    // 이때, var 로 선언된 변수 setter와 getter가 추가된다.
    public var value3 = 300
    // val로 선언된 변수는 getter만 추가된다.
    public val value4 = 400

    // 자동 생성되는 setter는 매개변수의 값을 그냥 맴버 변수에 저장하는 코드로 되어 있다.
    // 자동 생성되는 getter는 맴버 변수의 값을 반환하는 코드로 되어 있다.
    // 만약 자동 생성되는 setter나 getter 를 사용하지 않고 직접 정의하고 싶다면 다음과 같이 작성해준다.
    var value5 = 500
        // field : 변수를 의미한다.
        // getter
        get(){
...
//        }
    ...
    // 변수에 값을 저장하는 setter
    // 변수에 접근이 불가능하기 때문에 값을 직접 저장하는 것이 불가능하다.
    // 이에 setter를 통해 값을 저장해야 하는데 메서드 이므로 코드 작성이 가능하다.
