package com.myproject.tekit_kotlin_study.kotlin.Kotlin24_Interface

// 인터페이스

// 메서드 오버리아딩의 강제성을 주기위해 추상 클래스를 사용한다.
// 코틀린(자바 포함)은 단일 상속만 지원하기 때문에 사용해야할 추상 클래스가 여러 개라면
// 추상 클래스 하나당 하나의 자식클래스를 만들어줘야 한다.
// 만약 추상메서드를 가지고 있는 것이 인터페이스라면 하나의 클래스에 안에서 여러 인터페이스가 가진
// 추상 메서드를 오버라이딩 할 수 있다.

// 인터페이스도 상속 관계에서 객체의 주소값을 부모 타입의 변수에 담을 수 있는 것 처럼
// 인터페이스 타입의 변수에도 담을 수 있다. 이를 통해 오버라이딩한 메서드를 호출 할 수 있다.

// 클래스와 인터페이스의 차이는 인터페이스는 객체를 생성하기 위한 설계도가 아니다.
// 클래스와 인터페이스 관계에서 상속 관계가 성립할 수 없다.
// 또한 인터페이스를 가지고 객체를 생성하는 것이 불가능 하기 때문에 객체와 같이 데이터를 관리하는 것도 불가능
