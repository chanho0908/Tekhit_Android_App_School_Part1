...
fun main(){
    ...
    t1.testMethod1()
    println("-----------------------------------------")
    // 이미 생성되어 있는 객체에 코틀린 범위 지정 함수를 사용한다.

    // let : Any 상속
    val t2 = TestClass1(100, 200)
    // 첫 번째 매개변수로 객체의 주소값이 들어온다.
    // it을 통해 객체의 프로퍼티나 메소드를 사용하면 된다.
    t2.let {
        it.a3 = 300
        it.a4 = 400
        it.testMethod1()
    ...
    // this나 아무것도 통하지 않고 프로퍼티나 메소드를 사용하는 것이 가능하다.
    ...
    // let 이 반환하는 것이 담긴다.
    // let 마지막에 it 을 작성하여 객체의 주소값이 반환될 수 있도록 해줘야 한다.
    ...
    // this(혹은 생략) 을 통해 프로퍼티나 메소드에 접근할 수 있다.
    ...
    // 따라서 it을 통해 프로퍼티나 메소드에 접근할 수 있다.
    // also 람다식이 끝나면 변수(t10)에 객체의 주소값이 담기게 된다.
    // 마지막에 객체의 주소값이 반환하는 코드를 작성하지 않는다.
    ...
    println("-----------------------------------------------------")
    // 람다식 코드가 생성된 객체의 일부분 처럼 동작하기 때문에 this(혹은 생략)을 통해
    // 프로퍼티나 메소드에 접근한다.
