package com.myproject.tekit_kotlin_study.kotlin.Kotlin20_LateInit
// 지연 초기화
...
class TestClass1{
    // Kotlin 에서는 Property를 정의할 때 저장할 값을 무조건 지정해야 한다.
    var a1:Int = 100
    var a2 = 200
    // Property 정의시 저장할 값을 지정하지 않았지만
    // init 블럭이나 생성자에서 값을 저장하는 코드를 작성해 두면 오류가 발생하지 않다.
    // 이는 객체를 생성하면 무조건 init 블럭과 생성자의 코드가 동작하기 때문에
    // Property에 값이 저장되는 보장받을 수 있기 때문이다.
    var a3:Int
    var a4:Int

    // 지연 초기화
    // init 블럭이나 생성자에서 값을 저장하지 않고 메서드에서 값을 저장하는 경우
    // 사용한다.
    // var property만 가능하다.
    ...
    // property 정의시 저장할 값을 지정하지 않아도 된다.
    val a6:Int
    val a7:Int
